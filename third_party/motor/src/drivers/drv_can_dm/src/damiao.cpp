...
Motor_Control::Motor_Control(std::string bus_name, std::unordered_map<uint16_t, DmActData>* data_ptr)
    ...
    for (auto it = data_ptr_->begin(); it != data_ptr_->end(); ++it) {  // 遍历该bus下的所有电机
        std::shared_ptr<Motor> motor = std::make_shared<Motor>(
            it->second.motorType, it->second.mode, it->second.can_id, it->second.mst_id);
        addMotor(motor);
    }
    int thread_priority = 95;
    auto callback = boost::bind(&Motor_Control::canframeCallback, this, boost::placeholders::_1);
    while (!socket_can_.open(bus_name, callback, thread_priority)) {
...
void Motor_Control::control_mit(Motor &DM_Motor, float kp, float kd, float q, float dq,
                                float tau) {
    ...
    if (motors.find(id) == motors.end()) {
        std::string message = "[Error] In control_mit,no motor with id ";
        message += std::to_string(DM_Motor.GetCanId());
...
void Motor_Control::control_pos_vel(Motor &DM_Motor, float pos, float vel) {
    uint16_t id = DM_Motor.GetCanId();
    if (motors.find(id) == motors.end()) {
        std::string message = "[Error] In control_pos_vel,no motor with id ";
        message += std::to_string(DM_Motor.GetCanId());
        message += " is registered.";
        std::cerr << message << std::endl;
        std::exit(-1);  // 终止程序，返回非 0 表示错误
    ...
    uint8_t* vbuf = reinterpret_cast<uint8_t*>(&vel);
...
void Motor_Control::control_vel(Motor &DM_Motor, float vel) {
    uint16_t id = DM_Motor.GetCanId();
    if (motors.find(id) == motors.end()) {
        std::string message = "[Error] In control_vel,no motor with id ";
        message += std::to_string(DM_Motor.GetCanId());
        message += " is registered.";
        std::cerr << message << std::endl;
        std::exit(-1);  // 终止程序，返回非 0 表示错误
...
void Motor_Control::receive_param(uint8_t *data) {
    ...
    if (motors.find(canID) == motors.end()) {
        std::string message = "[Error] In receive_param,no motor with id ";
        message += std::to_string(canID);
        message += " is registered.";
        std::cerr << message << std::endl;
        std::exit(-1);  // 终止程序，返回非 0 表示错误
    }
    if (is_in_ranges(RID)) {
        ...
        data_uint32 |= static_cast<uint32_t>(data[7]) << 24;
        data_uint32 |= static_cast<uint32_t>(data[6]) << 16;
        data_uint32 |= static_cast<uint32_t>(data[5]) << 8;
        data_uint32 |= data[4];
...
bool Motor_Control::switchControlMode(Motor &DM_Motor, Control_Mode_Code mode) {
    ...
    if (motors.find(DM_Motor.GetCanId()) == motors.end()) {
        std::string message = "[Error] In switchControlMode,no motor with id ";
        message += std::to_string(DM_Motor.GetCanId());
        message += " is registered.";
        std::cerr << message << std::endl;
        std::exit(-1);  // 终止程序
...
bool Motor_Control::change_motor_param(Motor &DM_Motor, uint8_t RID, float data) {
    if (is_in_ranges(RID)) {
        ...
        uint8_t* data_uint8 = reinterpret_cast<uint8_t*>(&data_uint32);
    ...
    if (motors.find(DM_Motor.GetCanId()) == motors.end()) {
        std::string message = "[Error] In change_motor_param,no motor with id ";
        message += std::to_string(DM_Motor.GetCanId());
        message += " is registered.";
        std::cerr << message << std::endl;
        std::exit(-1);  // 终止程序，返回非 0 表示错误
...
void Motor_Control::write() {
    ...
    for (const auto& m : *data_ptr_) {  // 遍历该can接口下的所有电机
        ...
        if (motors.find(motor_id) == motors.end()) {
            std::string message = "[Error] In write,no motor with id ";
        ...
        if (t < motion_time) {
            ...
            if (!motion_completed) {
                ...
                /* 不置零点，根据 MIT 协议公式，目标位置与实际位置的差值小于 P_MAX，参数配置得当电机会正常动作 */
                // 等待一段时间后设置零点
...
void Motor_Control::read() {
    for (auto& m : *data_ptr_) {  // 遍历该can接口下的所有电机
        ...
        if (motors.find(motor_id) == motors.end()) {
            std::string message = "[Error] In read,no motor with id ";
            message += std::to_string(motor_id);
            message += " is registered.";
            std::cerr << message << std::endl;
            std::exit(-1);  // 终止程序，返回非 0 表示错误
        ...
        message += std::to_string(m.second.pos);
        message += " vel: ";
        message += std::to_string(m.second.vel);
        message += " effort: ";
        message += std::to_string(m.second.effort);
        std::cerr << message << std::endl;
    }
}
...
void Motor_Control::canframeCallback(const can_frame &frame) {
    ...
    // 当guard对象被创建时，它会自动锁定mutex_；当guard对象被销毁时，它会自动解锁mutex_
    ...
    static auto uint_to_float = [](uint16_t x, float xmin, float xmax, uint8_t bits) -> float {
        ...
        float data_value = data_norm * span + xmin;
        return data_value;
    };

    for (const auto& frame_stamp : read_buffer_) {
        can_frame local_frame = frame_stamp.frame;
        uint16_t canID = (static_cast<uint16_t>(local_frame.data[1]) << 8) | local_frame.data[0];

        if (read_write_save == true && motors.find(canID) != motors.end()) {
            // 这是发送保存参数或者写参数或者读参数返回的数据
            if (local_frame.data[2] == 0x33 || local_frame.data[2] == 0x55 || local_frame.data[2] == 0xAA) {
                // 发的是读参数或写参数命令，返回对应寄存器参数
                if (local_frame.data[2] == 0x33 || local_frame.data[2] == 0x55) {
                    receive_param(&local_frame.data[0]);
                }
                read_write_save = false;
            }
        } else {
            uint16_t q_uint = (static_cast<uint16_t>(local_frame.data[1]) << 8) | local_frame.data[2];
            uint16_t dq_uint = (static_cast<uint16_t>(local_frame.data[3]) << 4) | (local_frame.data[4] >> 4);
            uint16_t tau_uint = (static_cast<uint16_t>(local_frame.data[4] & 0xf) << 8) | local_frame.data[5];

            if (motors.find(local_frame.can_id) == motors.end()) {
                std::string message = "[Debug] Received frame with ID ";
                message += std::to_string(local_frame.can_id);
                message += " but not found in map.";
                std::cerr << message << std::endl;
                return;
            }
            auto m = motors[local_frame.can_id];
