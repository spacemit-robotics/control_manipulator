...
static int feetech_get_state(struct motor_dev *dev, struct motor_state *state) {
        ...
        // 位置转换 (0-4095 -> 弧度)
        state->pos =
                static_cast<float>(priv->data.cur_position);  // raw_position 单位： 0-4095 对应 0-2π rad
...
static int feetech_get_paras(struct motor_dev *dev, const void *address, void *out_data,
                                uint32_t data_len) {
        if (!dev || !dev->priv_data || !address || !out_data || data_len == 0)
                return -1;
        ...
        if (mem_addr + data_len > 71) {
                std::cerr << "[Feetech] Get parameters address out of bounds: "
                                    << static_cast<int>(mem_addr) << std::endl;
                return -1;
        }
        ...
        // 调用 SDK 提供的 Read() 方法
        int ret = priv->pack->get_sms_sts().Read(priv->motor_id, mem_addr,
...
static int feetech_set_paras(struct motor_dev *dev, const void *address, const void *data,
                                uint32_t data_len) {
        if (!dev || !dev->priv_data || !address || !data || data_len == 0)
                return -1;
        ...
        // 地址合法性检查
        if (mem_addr + data_len > 71) {
                std::cerr << "[Feetech] Set parameters address out of bounds: "
                                    << static_cast<int>(mem_addr) << std::endl;
                return -1;
        }
        ...
        if (is_read_only) {
                std::cerr << "[Feetech] Error: Cannot write to read-only register at "
                                    << static_cast<int>(mem_addr) << std::endl;
                return -1;
        }
        ...
        int ret = priv->pack->get_sms_sts().genWrite(
                priv->motor_id, mem_addr, reinterpret_cast<uint8_t *>(const_cast<void *>(data)),
                static_cast<uint8_t>(data_len));
